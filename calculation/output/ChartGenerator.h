...
class ChartSeries {
    ...
    public:
        ChartSeries(const std::string& id, unsigned int zindex, const std::string& type, 
                    const std::string& name, const std::string& color, const std::string& symbol, unsigned int y_axis, bool visible=true, const std::string& opacity="1")
                    : _id(id), _zindex(zindex), _type(type), _name(name), _color(color), _symbol(symbol), _y_axis(y_axis), _visible(visible), _opacity(opacity) {}
        ...
        std::string & toString(std::string& r) const {
            ...
            s << "{ id: '" << _id.c_str() << "', zIndex: " << _zindex << ", type: '" << _type.c_str() << "', name: '" << _name.c_str()
              << "', color: '#" << _color.c_str() << "', yAxis: " << _y_axis << ", visible:" << (_visible ? "true" : "false") << ", opacity: " << _opacity
